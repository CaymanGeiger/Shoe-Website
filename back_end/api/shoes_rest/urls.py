...
urlpatterns = [
    ...
    path(
        "items/<int:shoe_id>/<int:cart_id>/<int:quantity>/",
        CartList,
        name="CartListCreate",
    ),
    path(
        "item/<int:cart_id>/<int:cart_item_id>/", CartItemDelete, name="CartItemDelete"
    ),
    path("scrapesneaks/", scrape_sneaks, name="scrape_sneaks"),
]
