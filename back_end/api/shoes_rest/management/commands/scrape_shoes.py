...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            nargs="+",
            type=str,
            help='The keywords to scrape data for. E.g. "nike dunks" should be input as "nike" "dunks"',
        )
        parser.add_argument(
            "--limit", type=int, default=50, help="The number of products to fetch"
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        if response.status_code == 200:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully fetched and saved {len(products)} products."
                )
            )
        else:
            self.stdout.write(
                self.style.ERROR(f"Failed to fetch data for keyword: {keyword}")
            )
